/// character_state_shield();
 ...
 // Only run if we have a shield and we're not invincible.
    if(shield_index != 0 && invincibility == 0 && state != CS_SPRING)
    {
       // Fire shield.
       // ---------------------------------------------------------------------
          if(shield_index == SHIELD_FIRE)
          {
             if(shield_usable == true && input_action_pressed)
             {
                ...
                with(shield_object)
                {
                     ...
                     sprite_index = spr_shield_flames_burst;
                     alarm[0]     = 15;
                }
                shield_state       = 1;
                shield_usable      = false;
                ...
                audio_play(shield_use_fire, global.sfx_volume);   
             }
             if((shield_state == 1 && ground) || state == CS_SPRING)
             {
       ...
       // ---------------------------------------------------------------------       
          ...
          {             
             ...
             if(shield_state == 1 && ground)
             {
                ...
                audio_play(shield_use_bubble, global.sfx_volume);               
             }
             if(shield_state >= 1 && state == CS_SPRING || state == CS_HURT)
             {
                shield_state  = 0;
                shield_usable = 0;
                shield_object . shield_sprite_frame  = 0;
       ...
       // ---------------------------------------------------------------------     
          ...
          {             
             if(shield_usable == true && input_action_pressed)
             {
                ...
                shield_usable  = false;
                jump_lock      = false; 
                audio_play(shield_use_lightning, global.sfx_volume); 
                for(i=0; i<4; i+=1)
                {
