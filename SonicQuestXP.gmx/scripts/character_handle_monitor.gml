/// character_handle_monitor()
 ...
 // Destroy from the top.
    if(_monitor != noone)
    {
       ...
       {
          if(state = CS_JUMP || state = CS_ROLL || animation_index = "ROLL" || shield_state == 1)
          {
             // Destroy monitor:
                _monitor.parent = id;             
                with(_monitor)
 ...
 // Destroy from the side.
    if(_monitor != noone)
    {
       ...
       {
          if(state == CS_ROLL || shield_state == 1)
          {
             // Destroy monitor:
                _monitor.parent = id;                        
                with(_monitor)
 ...
 // Allow monitor to bounce on our head.
    ...
    if(monitor != noone && (y_speed == 0 && (state != CS_JUMP || state != CS_ROLL || animation_index != "ROLL")))
    {
       if(state != CS_CROUCH && state != CS_SPINDASH)
       {
